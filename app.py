...
st.caption(
    "📷 上傳實拍照 ➔ 🎯 選擇單一視覺亮點 ➔ 🚀 自動生成不變形的品牌廣告圖 Prompt"
)
...
uploaded_file = st.file_uploader(
    "請選擇眼鏡實拍照（系統將作為實物參考標的）", type=["jpg", "jpeg", "png"]
)

if uploaded_file is not None:
  img = Image.open(uploaded_file)
  st.image(
      img, caption="已載入實拍照（將保持此眼鏡型態不變形）", width=320
  )
...
with col_p1:
  product_name = st.text_input("商品名稱/型號", value="日光優選")
with col_p2:
  specs_text = st.text_input(
      "尺寸簡述", value="總寬14.2cm / 單片鏡寬5.5cm / 單片鏡高5cm / 重18g"
  )
...
if uploaded_file or st.button("🚀 生成廣告圖片 Prompt", type="primary"):
  ...
  prompt_en = (
      f"High-end commercial product advertisement photography of"
      f" {product_name}. CRITICAL REQUIREMENT: MUST strictly match the exact"
      " eyewear shape, frame structure, lens color, and proportions from the"
      " reference photo, zero distortion, zero alteration of product identity."
      f" Single core visual focus: {selected_spotlight}"
      f" ({spotlight_dict[selected_spotlight]}). Composition & atmosphere:"
      f" {comp_str}, clean negative space, minimal uncluttered layout, master"
      f" lighting, sharp focus on eyewear details, {bg_scene} background. 8k"
      " resolution, photorealistic, professional brand ad aesthetic, ultra-clean"
      " commercial look."
  )
  ...
  if st.button("✨ 立即繪製廣告圖"):
    if not hf_token:
      ...
      with st.spinner(
          "🤖 FLUX.1 正在生成廣告圖中，請稍候 10~20 秒..."
      ):
        try:
          # 使用 Hugging Face 官方 InferenceClient 自動連接最新 API 節點
